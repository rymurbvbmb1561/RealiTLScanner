...
func main() {
	...
	// Changed default output filename to include a timestamp would be nice, but keeping it simple
	flag.StringVar(&out, "out", "out.csv", "Output file to store the result")
	// Increased default timeout slightly for more reliable results on slow hosts
	...
	if out != "" {
		...
		defer f.Close()
		// Added SCAN_TIME column to help correlate results with when the scan was run
